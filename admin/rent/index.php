        ...
        <div class="container-fluid">
			<table class="table table-bordered table-stripped">
				...
				<tbody>
					<?php 
					$i = 1;
						$qry = $conn->query("SELECT o.*,concat(c.firstname,' ',c.lastname) as client from `rent` o inner join clients c on c.id = o.client_id order by unix_timestamp(o.date_created) desc ");

						$qry1 = $conn->query("UPDATE rent SET due_date = DATE_ADD(date_created, INTERVAL 7 DAY);");
...
<script>
	$(document).ready(function(){
		...
		$('.pay_order').click(function(){
			_conf("Are you sure to mark this order as paid?","pay_rent",[$(this).attr('data-id')])
		})
		$('.table').dataTable();
	})
	function pay_rent($id){
		start_loader();
		$.ajax({
			url:_base_url_+"classes/Master.php?f=pay_rent",
			method:"POST",
			data:{id: $id},
			dataType:"json",
			error:err=>{
