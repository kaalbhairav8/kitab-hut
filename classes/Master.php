...
Class Master extends DBConnection {
		...
		if($save_order){
			...
			if($save_olist){
				...
				if($this->capture_err())
					return $this->capture_err();
				$resp['status'] ='success';
				// $save_sales = $this->conn->query("INSERT INTO `rent` set $data");
			}else{
				$resp['status'] ='failed';
	...
	function rent_order(){
		...
		$data .= " ,paid = '{$paid}' ";
		$data .= " ,delivery_address = '{$delivery_address}' ";
		$order_sql = "INSERT INTO `rent` set $data";
		$save_order = $this->conn->query($order_sql);
		if($this->capture_err())
			return $this->capture_err();
		if($save_order){
	...
	function update_rent_status(){
		extract($_POST);
		$update = $this->conn->query("UPDATE `rent` set `status` = '$status' where id = '{$id}' ");
		if($update){
			$resp['status'] ='success';
			$this->settings->set_flashdata("success"," rent status successfully updated.");
		}else{
			$resp['status'] ='failed';
			$resp['err'] =$this->conn->error;
		}
	...
	function pay_rent(){
		extract($_POST);
		$update = $this->conn->query("UPDATE `rent` set `paid` = '1' where id = '{$id}' ");
		if($update){
			$resp['status'] ='success';
			$this->settings->set_flashdata("success"," rent payment status successfully updated.");
		}else{
			$resp['status'] ='failed';
			$resp['err'] =$this->conn->error;
		}
